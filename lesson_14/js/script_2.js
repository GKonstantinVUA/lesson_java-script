...
if (confirm('Почати тестування?')) {
	...
	let sumSponsorInvestment = webSitesCompany.reduce(
		(prevParamSum, currentSponsor) =>
			currentSponsor.sponsors.reduce(
				(prevTotalSum, currentValueSum) => currentValueSum.amountSponsorInvestment + prevTotalSum,
				0
			) > 10000
				? prevParamSum + 1
				: prevParamSum,
	...
	document.write(`<div class="result__box-data">
			<div class="result__name-value"> Кількість сайтів де спонсорські вкладення більші за 10000грн: <span class="result__answer-value">${sumSponsorInvestment}</span> одн.</div></div>`)
