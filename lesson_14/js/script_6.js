...
if (confirm('Почати тестування?')) {
	...
	let arrayObjectMinValueCondition = [];
	let arrayObjectMaxValueCondition = [];
	
	for (let itemForCondition of webSitesCompany) {
		if (itemForCondition.webSiteCost < 10000) arrayObjectMinValueCondition.push(JSON.parse(JSON.stringify(itemForCondition)));
		else arrayObjectMaxValueCondition.push(JSON.parse(JSON.stringify(itemForCondition)));
	}
	
	console.log(arrayObjectMinValueCondition)
	console.log(arrayObjectMaxValueCondition)
	
	document.write(`<div class="result__box-data">
		<div class="result__name-value"> Результат відображено у <span class="result__answer-value">console.log</span></div></div>`);
}