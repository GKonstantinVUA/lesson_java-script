...
if (confirm('Почати тестування?')) {
	...
	const findMaxProfit = webSitesCompany.reduce((prevParam, currentValueSearch) =>
		currentValueSearch.webSiteCost > prevParam.webSiteCost ? currentValueSearch : prevParam, webSitesCompany[0]
	);
	document.write(
		`<div class="result__box-data">
			<div class="result__name-value"> Максимальний прибуток був у <span class="result__answer-value">${findMaxProfit.yearRelease}</span> році - <span class="result__answer-value">${findMaxProfit.webSiteCost}</span> грн</div></div>`
	);
}